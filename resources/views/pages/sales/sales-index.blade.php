<x-layouts.app :breadcrumbs="$breadcrumbs ?? null">
    <x-slot:title>Sales List</x-slot:title>
    ...
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Sales List</h1>
                <p class="text-sm text-gray-500">Manage transaction invoices, dates, status, and detail records.</p>
            </div>
            <div>
                <a href="{{ route('sales.create') }}" class="inline-flex items-center gap-2 bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 text-sm font-semibold rounded-lg shadow-sm transition-colors duration-150 cursor-pointer">
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
            <div class="overflow-x-auto">
                <x-datatable 
                    ...
                    :headers="['Sales Code', 'Date', 'Grand Total', 'Status', 'Actions']"
                    :columns="[
    ...
    @push('scripts')
        ...
        <script>
            ...
            window['dt_ajax_sales-table'] = function(url) {
                return {
                    url: url,
                    data: function(d) {
        ...
        <script>
            ...
            function deleteSales(code) {
                Swal.fire({
                    title: 'Delete Sales',
                    text: `Are you sure you want to delete sales record "${code}"?`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            error: function(xhr) {
                                let message = 'An error occurred while deleting the sales record.';
